...
Telemetry._receiveForTest({source:globalThis,data:{type:'DREAM_GROUP_CONTEXT',data:{user_id:'must-not-pass',favorite_id:7,favorite_name:'예준',profile:{private:true},token:'must-not-pass',jwt:'must-not-pass',credential:'must-not-pass',secret:'must-not-pass'}}});
