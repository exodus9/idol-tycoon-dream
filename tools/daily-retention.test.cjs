...
assert.equal(Daily.pendingReturn(promiseEcho,'2026-08-03',{rid:7,runId:'run-7'}).runId,'run-7','the result hook must bind to its exact RUN even when another reply was queued first');
assert.equal(Daily.pendingReturn(promiseEcho,'2026-08-04',{rid:7,runId:'run-7'}),null,'a reserved reply stops being future-pending on its reveal day');
assert.equal(Daily.pendingReturn(promiseEcho,'2026-08-03',{rid:8}),null,'another idol must never inherit the return promise');
const claimedPromise=promiseEcho.map(x=>x.runId==='run-7'?{...x,claimedAt:2}:x);
assert.equal(Daily.pendingReturn(claimedPromise,'2026-08-03',{rid:7,runId:'run-7'}),null,'a claimed reply must not appear as a future contract');
const focusedFuture=Daily.focusedEchoState(promiseEcho,'2026-08-03',promiseEcho[1].id);
assert.equal(focusedFuture.pending.runId,'run-7','result-to-Daily navigation must keep the exact queued RUN even when an older echo exists');
assert.equal(focusedFuture.ready,null,'another ready echo must not replace the focused future RUN contract');
const focusedReady=Daily.focusedEchoState(promiseEcho,'2026-08-04',promiseEcho[1].id);
assert.equal(focusedReady.ready.runId,'run-7','the exact focused contract becomes the exact openable reply on reveal day');
