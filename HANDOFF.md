...
- **그룹카드**: 앞면 3인 사선(`ucardHTML` `members`+`.rph3`) + 뒷면 9명 그리드 크게(54px)

### 🔜 남은 것 (착수순서)
1. **IA**: 배틀로비를 차트탭 소속 1뎁스 최상단으로 이동(구조변경) · 탭 정체성(사무실=육성/운영, 차트=무대/경쟁)
2. **온보딩**: 튜토리얼 페이지별 상세화(`Tut` 모듈 이미 있음, 스포트라이트 코치마크) · 그룹생성+경쟁전 3판 후 공유 팝업
3. **업적 보상**: 클리어당 육성카드(R 위주, 큰 마일스톤만 SR — ⚠️합성경제 붕괴 방지)
4. **그룹카드**: 육성카드 상세 아이콘 확대
5. **UI 대비 전수**: 저대비 페이지(게임소개 등) 4.5:1 미달 싹 리스트업(눈대중 금지, 전수)
6. **포지션 능력치**: 2개+ 반영 명시하되 종류 비공개 + posScore 숫자→등급표현(적합/우수/최적)
7. **밸런스**: 시너지 한도(+40%) 삭제 + 전 항목 수치 대폭↓(⚠️뉘앙스2 세트=티어 승점컷 상대 백분위 재조정)
8. **연출 대공사(자산 필요)**: 랭킹(차트)전 무대화(내 카드들이 포지션별 순차 등장·기여 팝업, 픽셀 스프라이트 재활용) · BGM 3종(홈/육성/배틀, 첫 탭 후 재생) · 효과음(카드 깔 때 긴장·개화 반짝·승패 팡파레)
9. **데이터(선행확인)**: 검색 연습생을 최애돌 앱DB와 일 1회 동기화+프사 교체 — ⚠️정적사이트+사내망DB라 브라우저 직결 불가, 별도 배치(스냅샷→idols.js 갱신) 설계 필요 · 연습생 특성은 나무위키 수작업 매핑(좀 틀려도 OK)
10. **최애돌 서사 침투(상시)**: 게임 속 연예계가 최애돌 앱 중심(하트픽·테마픽·이달의기적·기부요정·프리톡) — 매판 나열 금지, 이벤트당 1개 로테이션 + 실앱 이름/아이콘 통일
